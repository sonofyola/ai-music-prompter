...
if (typeof window !== 'undefined' && typeof window.ExponentConstants === 'undefined') {
  try {
    ...
    console.error('[Polyfills] Error setting up expo-constants polyfill:', error);
  }
}
    ...
    try {
      ...
      if (!('structuredClone' in global)) {
        try {
          ...
          console.warn('[Polyfills] Failed to load structuredClone polyfill:', cloneError);
        }
      }
      ...
      try {
        ...
        console.warn('[Polyfills] Failed to load text encoding streams:', streamError);
      }
      
      console.log('[Polyfills] Mobile platform - polyfills applied');
    } catch (error) {
      console.warn('[Polyfills] Polyfill setup failed:', error);
    }
  };

  setupPolyfills().catch(error => {
    console.error('[Polyfills] Failed to setup mobile polyfills:', error);
  });
} else {
